...
- **HAProxy** - для распределения HTTP-запросов
- **ProxySQL** - для балансировки SQL-запросов между шардами
- **MySQL Router** - для автоматической маршрутизации запросов

### Мониторинг:
- **Prometheus + Grafana** - метрики производительности
- **Percona Monitoring Tools** - мониторинг MySQL
- **Orchestrator** - управление топологией репликации

### Управление шардами:
- **Vitess** - для автоматического шардинга (опционально)
- **MyLoader/MyDumper** - для миграции данных между шардами
...
## 9. Примеры SQL-запросов с маршрутизацией

### Запросы к таблице `пользователи`:
```sql
-- Чтение: определяется по user_id
SELECT * FROM users WHERE user_id = 123; -- → Шард 2 (123 % 3 = 0)

-- Запись: аналогичная логика
INSERT INTO users (user_id, name) VALUES (456, 'John'); -- → Шард 3 (456 % 3 = 0)
```

### Запросы к таблице `книги`:
```sql
-- Поиск по жанру (определяет шард)
SELECT * FROM books WHERE genre = 'Fantasy'; -- → Подшард 1 (A-M)
SELECT * FROM books WHERE genre = 'Science'; -- → Подшард 2 (N-Z)

-- Запись с определением шарда по genre
INSERT INTO books (title, genre) VALUES ('Book Title', 'Romance'); -- → Подшард 2
```

### Запросы к таблице `магазины`:
```sql
-- По региону
SELECT * FROM shops WHERE region_id = 2; -- → Подшард 2
UPDATE shops SET address = 'New Address' WHERE shop_id = 15 AND region_id = 3;
```

**Логика маршрутизации:**
- Балансировщик анализирует WHERE-условия
- Определяет целевой шард по ключу шардинга
- Направляет запрос на соответствующий Master/Slave

## 10. Блоксхема системы

```mermaid
flowchart TD
    A[Клиентские запросы] --> B[Балансировщик нагрузки]
    B --> C[Вертикальный шардинг]
    
    subgraph C[Вертикальный шардинг]
        subgraph D[Шард «пользователи»]
            D1[Подшард 1<br/>user_id % 3 = 0]
            D2[Подшард 2<br/>user_id % 3 = 1]
            D3[Подшард 3<br/>user_id % 3 = 2]
        end
        
        subgraph E[Шард «книги»]
            E1[Подшард 1<br/>жанр A–M]
            E2[Подшард 2<br/>жанр N–Z]
        end
        
        subgraph F[Шард «магазины»]
            F1[Подшард 1<br/>регион 1]
            F2[Подшард 2<br/>регион 2]
            F3[Подшард 3<br/>регион 3]
            F4[Подшард 4<br/>регион 4]
        end
    end
    
    D1 --> G[Реплики Slave<br/>для каждого шарда]
    D2 --> G
    D3 --> G
    E1 --> G
    E2 --> G

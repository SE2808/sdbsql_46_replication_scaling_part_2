...
flowchart TD
    ...
    C --> D[Шард «пользователи»]
    D --> D1[Подшард 1 (user_id % 3 = 0)]
    D --> D2[Подшард 2 (user_id % 3 = 1)]
    D --> D3[Подшард 3 (user_id % 3 = 2)]
    
    C --> E[Шард «книги»]
    E --> E1[Подшард 1 (жанр A–M)]
    E --> E2[Подшард 2 (жанр N–Z)]
    
    C --> F[Шард «магазины»]
    F --> F1[Подшард 1 (регион 1)]
    F --> F2[Подшард 2 (регион 2)]
    F --> F3[Подшард 3 (регион 3)]
    F --> F4[Подшард 4 (регион 4)]
    
    D --> G[Реплики (Slave) для шарда «пользователи»]
    E --> H[Реплики (Slave) для шарда «книги»]
    F --> I[Реплики (Slave) для шарда «магазины»]
